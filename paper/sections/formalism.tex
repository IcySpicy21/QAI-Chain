...
The PPO branch uses a clipped surrogate objective \cite{schulman2017ppo}:
\begin{equation}
...
\end{equation}
Normalization can improve conditioning in mini-batch gradient steps by reducing scale variance across updates \cite{sutton2018reinforcement}. In our environment, removing it can increase average reward in some seeds, but with larger instability under policy drift.
...
We keep strong simple baselines. In constrained control surfaces, proportional heuristics can dominate RL policies when target structure is directly exposed. Reporting this outcome directly is necessary for scientific validity and empirical comparison \cite{pineau2021improving}.
